...
def main():
    ...
    # run = 'run_18'
    # subrun = 'resist_430V_drift_600V'
    run = 'run_33'
    subrun = 'initial_resist_440V_drift_600V'
    file_num = 0
    event = 8

    run_dir = os.path.join(run_dir, run, subrun, 'decoded_root')
    ...
    feus = {'x': 4, 'y': 5}
    evt_samples_xy, evt_channels_xy, evt_amplitudes_xy = {}, {}, {}

    for orientation, feu_num in feus.items():
        ...
        with uproot.open(file_path) as f:
            ...
            evt_samples_xy[orientation] = evt_samples
            evt_channels_xy[orientation] = evt_channels
            evt_amplitudes_xy[orientation] = evt_amplitudes
    ...
    plot_waveforms_with_x17(
        ...
        blind_time_s=1250e-9,  # adjust by eye
...
def plot_waveforms_with_x17_nov(
...
def plot_waveforms_with_x17(
        evt_channels_x, evt_samples_x, evt_amplitudes_x,
    ...
    # Convert sample # → time
    # ---------------------------
    evt_time_x = evt_samples_x * sample_period_s   # time in seconds
    evt_time_y = evt_samples_y * sample_period_s   # time in seconds
    ...
    ax_cs.axhline(0, color="gray", linestyle="-", zorder=0)
    ax_cs.set_xlim(left=0, right=np.max(evt_time_x) * 1e6 * 1.05)
    ax_cs.set_ylabel("X17 Yield (x17/day)")
    ...
    ax_top.set_xlim(ax_cs.get_xlim())
    ...
    for ch in unique_x:
        m = evt_channels_x == ch
        ax_x.plot(evt_time_x[m]*1e6, evt_amplitudes_x[m], lw=0.7)
    ...
    for ch in unique_y:
        m = evt_channels_y == ch
        ax_y.plot(evt_time_y[m]*1e6, evt_amplitudes_y[m], lw=0.7)
