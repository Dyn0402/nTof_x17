...
BASE_DIR = "/media/dylan/data/x17/feb_beam/dream_run"
DECODER_PATH = '/home/dylan/CLionProjects/mm_strip_reconstruction/cmake-build-debug/decoder/decode'
CONVERTER_PATH = '/home/dylan/CLionProjects/beam_test_2023/decode/convert_vec_tree_to_array'


def main():
    # runs = ['run_60', 'run_61', 'run_62']  # List of runs to decode
    runs = ['run_17']  # List of runs to decode
    for run in runs:
        run_dir = os.path.join(BASE_DIR, run)
        for subrun in os.listdir(run_dir):
            # if subrun != 'resist_hv_500V' and subrun != 'resist_hv_505V':
            #     continue
            # run = 'run_50'
            # run = 'test_tcm'
            # Find all .fdf files in the run directory
            subrun_dir = os.path.join(run_dir, subrun)
            fdf_files = [f for f in os.listdir(subrun_dir) if f.endswith('.fdf')]
            print(f'Found {len(fdf_files)} .fdf files in {subrun_dir}')

            # Iterate through and decode each .fdf file
            for fdf_file in fdf_files:
                fdf_path = os.path.join(subrun_dir, fdf_file)
