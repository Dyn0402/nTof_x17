#!/usr/bin/env python3
"""
A1 — run_64 singles 2-D scan: track yield vs time-since-flash and HV, per det.

The singles DAQ accepts in a rigid deadtime comb. Measured on run_64 (see
scan_lib): the flash trigger at dt=0, then 4 events at ~5.0 ms (front-end
BLIND), 2 at ~13.5 ms (partially recovered), and 2 each at ~27/41/55/69 ms
(recovered) -> ~15.1 events/spill. Probe classes:
  early = the ~5.0 ms batch   (first reconstructable events after the flash)
...
run_58's narrower (20,33) ms window could reach.

MESH: injection is live on A and C all run; B and D are the uncabled control.
See scan_lib for why A/C-vs-B/D is a confounded comparison and run_61 is the
proper mesh-off baseline (fig_vs_run61).
...
Run: .venv/bin/python ntof_july_analysis/run64_scan/analyze_tracks.py
Output -> <ANALYSIS_DIR>/July_HV_Scan/run64_scan/tracks/
"""
import os
import sys
...
IDX_MAX = 15
IDX_DT_MS = {0: 0.0, 1: 5.0, 2: 5.0, 3: 5.1, 4: 5.1, 5: 13.4, 6: 13.6,
...
def load():
    ...
    # Attach per-event FEU-presence flags; every per-detector rate below is
    # evaluated on that detector's live events only. run_64 was taken entirely
    # AFTER the processor_watcher fixes, so it should be ~100% live — the cut is
    # kept as a guard and as an apples-to-apples match to run_61's treatment.
    ev = FP.attach(ev)
    if not segs.empty:
...
def fig_time_recovery(st):
    ...
    fig.suptitle('run_64 singles — post-flash recovery of track yield '
...
CLS_TITLE = {
    'early': ('5 ms group — the 4 triggers at ~5.0 ms after the gamma flash '
              '(PRIMARY)'),
...
def fig_yield_vs_hv(st, cls='late'):
    ...
    fig.suptitle(f'run_64 — {CLS_TITLE[cls]}\ntrack yield vs HV '
...
def fig_slot_recovery(ev):
    ...
    The 3-class ladder collapses 15 accept slots into 3 points; run_64's comb
    ...
    fig.legend(loc='center right', fontsize=8, title='resist HV\n(A/B/C)')
    fig.suptitle('run_64 — post-flash recovery at full comb resolution '
                 '(drift-pooled, per resist)', fontsize=12)
    fig.tight_layout(rect=(0, 0, 0.9, 1))
    return _save(fig, 'slot_recovery.png')


# run_61 = the MESH-OFF baseline: identical trigger, identical DAQ config, same
# probe-class definitions, taken the night before. That is the comparison that
# isolates the mesh, so it replaces run58_scan's overlay here.
RUN61_STATS = os.path.join(os.path.dirname(L.OUT_BASE), 'run61_scan', 'tracks',
                           'per_cell_stats.csv')


def fig_vs_run61(st, cls='late'):
    """Overlay run_64 (mesh ON for A/C) on run_61 (mesh OFF), at shared drifts.

    This is the mesh test: A and C should move if the mesh does anything, while
    B and D — uncabled in run_64 and identical in configuration between the two
    runs — should NOT. B/D therefore act as the run-to-run null: any shift seen
    there is a systematic (beam, gas, time) rather than a mesh effect, and must
    be subtracted from the A/C shift before believing it.

    Resist overlap is 560..520 V (run_61 560->515, run_64 570->520); the 565-570
    points are run_64-only, and are exactly the region run_61 could not reach.
    """
    if not os.path.exists(RUN61_STATS):
        print(f'  (skipping run_61 overlay — {RUN61_STATS} not found)')
        return None
    s61 = pd.read_csv(RUN61_STATS)
    shared = sorted(set(st.drift.unique()) & set(s61.drift.unique()))
    fig, axes = plt.subplots(2, 2, figsize=(12.5, 8.5), sharex=True)
    for ax, Ld in zip(axes.flat, 'ABCD'):
        for src, s, col, ls, mk in (('run_61 (mesh OFF)', s61, 'gray', '--', 's'),
                                    ('run_64 (mesh A,C)', st, DET_COL[Ld], '-', 'o')):
            g = s[(s.det == Ld) & (s.probe_class == cls) & s.drift.isin(shared)]
            if g.empty:
                continue
        ...
        tag = ' — MESH' if Ld in L.MESH_DETS else ' — control'
        ax.set_title(f'Det {Ld}{tag}' + (' (clean M1)' if Ld == 'A' else ''),
                     fontsize=10, color=DET_COL[Ld])
        ax.grid(alpha=0.3)
    for ax in axes[:, 0]:
        ax.set_ylabel(f'P(3D x/y pair | {cls})')
    for ax in axes[1]:
        ax.set_xlabel('resist HV [V]  (det D = setpoint - 10)')
    fig.legend(loc='center right', fontsize=8)
    fig.suptitle(f'run_64 (mesh on A,C) vs run_61 (mesh off) — {cls} track yield '
                 f'vs resist (drifts {shared} pooled).  A/C = mesh, B/D = null.',
                 fontsize=11)
    fig.tight_layout(rect=(0, 0, 0.88, 1))
    return _save(fig, f'vs_run61_{cls}.png')
...
def fig_gain_vs_hv(st):
    ...
    fig.suptitle('run_64 — gain proxy (late-probe track charge) vs HV', fontsize=12)
    fig.tight_layout()
...
def fig_liveness(st):
    ...
    fig.legend(loc='center right', fontsize=8, title='resist HV')
    fig.suptitle('run_64 — front-end liveness vs probe class (drift-pooled)',
                 fontsize=12)
    fig.tight_layout(rect=(0, 0, 0.9, 1))
...
def main():
    ...
    figs += [fig_gain_vs_hv(st), fig_liveness(st)]
    figs += [fig_vs_run61(st, c) for c in CLASSES]   # the mesh test
    for f in figs:
        if f:
            print('  ->', f)
