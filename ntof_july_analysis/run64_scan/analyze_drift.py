#!/usr/bin/env python3
"""
A2 — run_64 singles 2-D scan: drift velocity, effective gap, efficiency vs
drift & resist HV.
...
plotted so the edge definitions are auditable):
  ...
  T_max (full-gap transit = gap / v_drift):  high percentile (default P95) of
    ...
    -> 1975 ns @200 V; run_64 spans the same 700-200 V range, so its curve is a
...
run_64 grid: 6 drift points (700, 600, 500, 400, 300, 200 V) x 10 resist
...
Run: .venv/bin/python ntof_july_analysis/run64_scan/analyze_drift.py
Output -> <ANALYSIS_DIR>/July_HV_Scan/run64_scan/drift/
"""
import json
import os
import sys
...
def load():
    ...
    ev = FP.attach(ev)      # run_64 FEU-dropout flags (efficiency denominators)
...
def fig_tspan_dist(segs):
    ...
    fig.suptitle(f'run_64 — micro-TPC track tspan vs drift HV '
...
def fig_drift_spectrum(spec):
    ...
    fig.legend(loc='center right', fontsize=8, title='drift HV')
    fig.suptitle('run_64 — clean-hit charge arrival spectrum vs drift '
...
def fig_vdrift(summ):
    ...
    fig.suptitle('run_64 — measured drift velocity & full-gap transit time '
...
def fig_efficiency(ev):
    ...
    for det in 'ABCD':
        ...
        for dr, g in late.groupby('drift'):
            g = g[g[f'readout_{det}']]          # FEU-dropout correction (run_64)
    ...
    fig.suptitle('run_64 — track-finding efficiency vs drift HV', fontsize=12)
    fig.tight_layout()
