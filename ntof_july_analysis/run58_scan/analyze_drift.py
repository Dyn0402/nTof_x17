#!/usr/bin/env python3
"""
A2 — run_58 singles 2-D scan: drift velocity, effective gap, efficiency vs
drift & resist HV.
...
plotted so the edge definitions are auditable):
  ...
  Efficiency:  P(3D x/y pair | late probe) per trigger vs drift (and resist).
    ...
    reference (inter-chamber pointing, follow-up).

Run: .venv/bin/python ntof_july_analysis/run58_scan/analyze_drift.py
Output -> <ANALYSIS_DIR>/July_HV_Scan/run58_scan/drift/
"""
import json
import os
import sys
...
def fig_tspan_dist(segs):
    ...
    fig.suptitle(f'run_58 — micro-TPC track tspan vs drift HV '
...
def fig_drift_spectrum(spec):
    ...
    fig.legend(loc='center right', fontsize=8, title='drift HV')
    fig.suptitle('run_58 — clean-hit charge arrival spectrum vs drift '
...
def fig_vdrift(summ):
    ...
    fig.suptitle('run_58 — measured drift velocity & full-gap transit time '
...
def fig_efficiency(ev):
    ...
    fig.suptitle('run_58 — track-finding efficiency vs drift HV', fontsize=12)
    fig.tight_layout()
