#!/usr/bin/env python3
"""
A3 — run_58 singles 2-D scan: best drift/resist operating point per detector.
...
figure of merit:
  ...
  live_frac  = post-flash live fraction of the 3-30 ms window, from the mid/late
               recovery ladder (pooled over drift per (det,resist) for stats):
                 mid ~ late (ratio>0.7) -> recovered by ~13 ms  -> live ~ 0.85
                 mid partially back      -> recovery 13-22 ms   -> live ~ 0.55
                 mid ~ 0                  -> recovery >22 ms      -> live ~ 0.30
...
Run: .venv/bin/python ntof_july_analysis/run58_scan/optimize.py
Output -> <ANALYSIS_DIR>/July_HV_Scan/run58_scan/optimize/
"""
import os
import sys
...
def fig_heatmaps(surf, valcol, label, fname, cmap='viridis'):
    ...
    fig.suptitle(f'run_58 — {label}  (kernel-smoothed; box = 1-sigma plateau, '
...
def fig_profiles(surf):
    ...
    fig.suptitle('run_58 — efficiency vs drift at each detector\'s optimum resist '
...
def recommendation(surf):
    ...
    lines = ['# run_58 — recommended drift / resist operating point per detector',
             '',
             ...
             'A/B/C setpoint - 10 V.',
             '',
             'Drift guidance: v_drift saturates above ~500-600 V (analyze_drift) '
             'and the full 30 mm gap is active at every drift; efficiency rolls '
             'off slightly at 700 V (fast drift compresses the column). So the '
             'sweet spot is the v-saturated end of the efficiency plateau, '
             'typically **500-600 V**.',
             '']
    rows = []
    for det in 'ABCD':
        ...
        note = {'A': 'clean M1 — reference detector, HIGH confidence.',
                'B': 'bad M1, noise-dominated (eff ~1%): LOW confidence — treat '
                     'as "run at max available gain", tracking marginal.',
                'C': 'bad M1 but usable; efficiency still climbing with drift, so '
                     'favour the high-drift end.',
