#!/usr/bin/env python3
"""
A3 — run_62 singles 2-D scan: best drift/resist operating point per detector.
...
figure of merit:
  ...
  live_frac  = post-flash live fraction, from the mid/late recovery ladder
               (pooled over drift per (det,resist) for stats):
                 mid ~ late (ratio>0.7) -> recovered by ~13.6 ms -> live ~ 0.85
                 mid partially back      -> recovery 14-27 ms    -> live ~ 0.55
                 mid ~ 0                  -> recovery >27 ms      -> live ~ 0.30

SCOPE, vs run_58: run_62 was truncated at 3 h, so the 'surface' is really one
complete resist pass (560->520 V, 9 pts) at drift 700 V plus a 4-point stub at
drift 300 V. The kernel bandwidth (85 V) is far narrower than the 400 V drift
spacing, so the two drift columns are smoothed INDEPENDENTLY — read the result
as "best resist at drift 700" with a 300 V sanity column, not as a 2-D optimum.
The resist window also sits 20 V BELOW run_58's (580->540), overlapping only on
560..540 V; run_58 remains the authority above 560 V.
...
Run: .venv/bin/python ntof_july_analysis/run62_scan/optimize.py
Output -> <ANALYSIS_DIR>/July_HV_Scan/run62_scan/optimize/
"""
import os
import sys
...
# drift velocity saturates above ~500-600 V (run_58's analyze_drift; run_62 has
# only 2 drift points and cannot re-measure the knee): below this the drift
# column is long (diffusion/pileup) and v is field-sensitive. Reference line only.
V_SAT_DRIFT = 500.0
...
# --------------------------------------------------------------------------- #
def kn_grid(st, det, metric='pair'):
    """(drifts, resists, K, N) count grids for the late probe, one detector."""
    s = st[(st.det == det) & (st.probe_class == 'late')]
    ...
    for i, r in enumerate(resists):
        for j, d in enumerate(drifts):
            ...
            if not cell.empty:
                K[i, j] = round(float(cell[pcol].iloc[0]) * float(cell.n.iloc[0]))
                N[i, j] = float(cell.n.iloc[0])
...
def fig_heatmaps(surf, valcol, label, fname, cmap='viridis'):
    ...
    fig.suptitle(f'run_62 — {label}  (kernel-smoothed; box = 1-sigma plateau, '
...
def fig_profiles(surf):
    ...
    for ax, det in zip(axes.flat, 'ABCD'):
        ...
        ax.set_ylabel('P(3D pair | late)')
        ax.grid(alpha=0.3); ax.legend(fontsize=7)
    fig.suptitle('run_62 — efficiency vs drift at each detector\'s optimum resist '
                 '(shaded = 1-sigma plateau; dotted = v_drift saturation ~500 V)',
                 fontsize=12)
    fig.tight_layout()
    p = os.path.join(OUT, 'profiles.png')
    fig.savefig(p, dpi=130)
    plt.close(fig)
    return p
...
# --------------------------------------------------------------------------- #
def build_surfaces(st):
    rec = recovery_by_resist(st)
    surf = {}
    for det in 'ABCD':
        drifts, resists, K, N = kn_grid(st, det, 'pair')
...
def suggest_setpoint(oe, of, det):
    """Physics-informed single setpoint from the efficiency plateau + FOM.

    Drift: run_58's version interpolated onto the v_drift-saturation knee
    (~500 V), which it could do because 500 V was a scanned point. run_62 only
    has {700, 300}, so we do NOT invent an unmeasured setpoint — we report the
    better of the two measured columns and defer the real drift choice to
    run_58. Resist: efficiency-optimum, pulled toward the FOM resist only if the
    FOM prefers a materially lower gain (live-window win).
    """
    drift = oe['drift']
...
def recommendation(surf):
    """Per-detector written operating-point recommendation."""
    lines = ['# run_62 — recommended drift / resist operating point per detector',
             '',
             'Primary metric = **P(3D x/y pair | late/recovered probe)** per '
             ...
             'A/B/C setpoint - 10 V.',
             '',
             '**Scope.** run_62 (singles-only, no PS/flash trigger) was '
             'truncated at 3 h: a complete resist pass 560->520 V at drift '
             '700 V, plus a 4-point stub (560->545 V) at drift 300 V. The '
             'drift axis is therefore a 2-point comparison, not a scan — take '
             'the drift recommendation from run_58 (7 points, 700->200 V: '
             'v_drift saturates above ~500-600 V, so the sweet spot is '
             '500-600 V) and read run_62 for the RESIST optimum, which it '
             'extends 20 V lower than run_58 could.',
             '']
    rows = []
    for det in 'ABCD':
        d = surf[det]
...
def main():
    ...
    surf, rec = build_surfaces(st)

    print('  ->', fig_heatmaps(surf, 'P',
                               'recovered-window efficiency P(3D pair | late)',
                               'eff_smooth_heatmap.png'))
    print('  ->', fig_heatmaps(surf, 'fom',
                               'operating FOM = eff_late x live_frac',
                               'fom_smooth_heatmap.png', cmap='magma'))
    print('  ->', fig_profiles(surf))

    text, bp = recommendation(surf)
    with open(os.path.join(OUT, 'recommendation.md'), 'w') as f:
        f.write(text)
    bp.to_csv(os.path.join(OUT, 'best_points.csv'), index=False)
    print('  -> ' + os.path.join(OUT, 'recommendation.md'))
    print('\n' + text)
