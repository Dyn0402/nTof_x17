#!/usr/bin/env python3
"""
A2 — run_62 singles 2-D scan: drift velocity, effective gap, efficiency vs
drift & resist HV.
...
plotted so the edge definitions are auditable):
  ...
  T_max (full-gap transit = gap / v_drift):  high percentile (default P95) of
    ...
    unambiguous and monotonic in drift HV (972 ns @700 V -> 1975 ns @200 V).
  ...
  Efficiency:  P(3D x/y pair | late probe) per trigger vs drift (and resist).
    ...
    reference (inter-chamber pointing, follow-up). 'late' here = comb teeth
    2-5 (27-68 ms after the spill/flash), see scan_lib.

CAVEAT vs run_58: run_62 was truncated at 3 h, so the drift axis has only TWO
points (700 and 300 V) and the 300 V column only covers resist 560-545. The
v_drift / effective-gap figures are therefore a 2-point CHECK against run_58's
7-point curve (700..200 V), not a standalone measurement.

Run: .venv/bin/python ntof_july_analysis/run62_scan/analyze_drift.py
Output -> <ANALYSIS_DIR>/July_HV_Scan/run62_scan/drift/
"""
import json
import os
import sys
...
def fig_tspan_dist(segs):
    ...
    fig.suptitle(f'run_62 — micro-TPC track tspan vs drift HV '
...
def fig_drift_spectrum(spec):
    ...
    fig.legend(loc='center right', fontsize=8, title='drift HV')
    fig.suptitle('run_62 — clean-hit charge arrival spectrum vs drift '
...
def fig_vdrift(summ):
    ...
    fig.suptitle('run_62 — measured drift velocity & full-gap transit time '
...
def fig_efficiency(ev):
    ...
    for i, r in enumerate(resists):
        rows = []
        for dr, g in late[late.resist == r].groupby('drift'):
    ...
    fig.suptitle('run_62 — track-finding efficiency vs drift HV', fontsize=12)
    fig.tight_layout()
