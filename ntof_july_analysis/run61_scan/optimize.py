#!/usr/bin/env python3
"""
A3 — run_61 singles 2-D scan: best drift/resist operating point per detector.
...
Grid: run_61 is a COMPLETE 6 drift (700..200 V) x 10 resist (560..515 V) scan,
...
Run: .venv/bin/python ntof_july_analysis/run61_scan/optimize.py
Output -> <ANALYSIS_DIR>/July_HV_Scan/run61_scan/optimize/
"""
import os
import sys
...
def kn_grid(st, det, metric='pair', cls='late'):
    ...
    for i, r in enumerate(resists):
        for j, d in enumerate(drifts):
            ...
            # After the run_61 FEU-dropout correction a cell can be genuinely
...
def fig_heatmaps(surf, valcol, label, fname, cmap='viridis'):
    ...
    fig.suptitle(f'run_61 — {label}  (kernel-smoothed; box = 1-sigma plateau, '
...
def fig_profiles(surf, cls='late'):
    ...
    fig.suptitle(f'run_61 ({cls}) — efficiency vs drift at each detector\'s '
                 'optimum resist '
...
def recommendation(surf, cls='late'):
    ...
    lines = [f'# run_61 ({cls}) — recommended drift / resist operating point '
             'per detector',
             '',
             ...
             'suggested setpoint. '
             'run_61 scans 700..200 V in 6 steps and 560..515 V in resist; the '
             'resist window is 20 V below run_58\'s, so run_61 is the authority '
...
def main():
    ...
    # run_61 FEU dropouts: per_cell_stats evaluates each detector only on its
