...
| `event_displays_3d/` | 3-D single-event displays: the drift-time charge cloud with the independent M3 reference track drawn as a line (static PNG + rotating GIF per event). `DISPLAYS_3D.md` lists them. The polished figures used in the report/deck are `figures/07` (hero) and `figures/08` (gallery). |
...
predate the recipe change and are flagged as such in the report). The slide deck
(`slides/mx17_detector_performance.pptx`) is also rebuilt (18 slides) with every headline number reconciled the same way,
