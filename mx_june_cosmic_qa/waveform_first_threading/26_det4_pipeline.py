#!/usr/bin/env python3
"""Portability test: full waveform-first protocol on det4 (g_det4 day run).

Stages (all outputs under <det4 Analysis>/mx17_4/waveform_first/):
...
def build_cache():
    ...
    print(f'{len(events)} matched det4 events', flush=True)
    ...
    print(f'{len(events)} det4 events cached', flush=True)
    return events
...
def build_templates(events):
    ...
    for plane in ('x', 'y'):
        ...
        for ev in events.values():
            ...
            if tn < 0.20:                      # det4 lower stats: looser
                continue
        ...
        print(f'det4 template {plane}: n={len(acc)}', flush=True)
...
if __name__ == '__main__':
    ...
    fm3._TT_CACHE.clear()
    print(f'det4 window: {ns_det} samples', flush=True)
    ...
    print(f'det4: {len(cand)} candidates, train {len(train)}, test {len(test)}',
          flush=True)
    ...
    c0 = total_chi2(x0)
    print(f'det4 initial chi2 {c0:.4e}', flush=True)
    neval = [0]
    def obj(x):
        x = np.asarray(x)
    ...
    json.dump(out, open(os.path.join(OUT_DIR, 'hyper_det4.json'), 'w'), indent=1)
    print('det4 hypers:', np.round(res.x, 3), flush=True)
    ...
    print(f'det4 freefit {len(resf)} in {time.time()-t_:.0f}s', flush=True)
