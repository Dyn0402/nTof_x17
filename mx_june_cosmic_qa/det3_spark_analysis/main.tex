...
\section{Q2b --- Are the ``no-track'' sparks really particle showers?  No}
A natural competing explanation for the no-track population is a \textbf{shower}: a burst
of several particles that crosses both M3 (leaving a mess that fails to fit a single
track) and our detector (firing many strips, read as a spark). This is directly testable,
because a shower of several charged particles leaves M3 with \emph{multiple straight-track
candidates} ($\text{ncand}\ge2$), whereas a corrupted or particle-free event leaves
$\text{ncand}=0$ (M3 blank) or a single degraded candidate ($\text{ncand}=1$). We count
raw M3 track candidates per event (before quality cuts) and split by spark class
(Fig.~\ref{fig:shower}):
%
\begin{table}[h]\centering\small
\begin{tabular}{lccc}
\toprule
event class & ncand $=0$ (blank) & ncand $=1$ (single) & ncand $\ge2$ (\textbf{shower})\\
\midrule
non-spark        & 17.5\,\% & 82.3\,\% & 0.15\,\%\\
all sparks       & 34.1\,\% & 64.9\,\% & 1.02\,\%\\
\textbf{no-track sparks} & 56.5\,\% & 42.2\,\% & \textbf{1.26\,\%}\\
\bottomrule
\end{tabular}
\end{table}
%
The shower signature is essentially absent: only \textbf{1.3\,\%} of no-track sparks have
$\ge2$ M3 tracks (det7: 0.5\,\%), and the M3 tracker never returns more than three
candidates --- multi-track events are $0.2\,\%$ of the entire run, far too rare to
account for the thousands of no-track sparks. Instead the no-track sparks are M3-blank
(57\,\%) or a single track that failed the quality cut (42\,\%). Second, the
\textbf{discharge morphology is identical} for muon-tracked and no-track sparks (median
88 vs 91 strips, span 304 vs 372~mm, time spread 251 vs 257~ns, edge and saturation
fractions equal; Fig.~\ref{fig:shower}c) --- the no-track sparks are the \emph{same
physical discharge} as the muon-induced ones, not a distinct multi-cluster shower
signature. \textbf{The shower hypothesis is ruled out.}

What the split \emph{does} show is that a spark nearly \textbf{doubles} the chance that M3
returns no track at all ($17.5\to34\,\%$ blank): the discharge suppresses M3
reconstruction, on top of a genuinely muon-free spontaneous component. Q2c pins this down
directly from the M3 raw data.

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{fig_shower_test.png}
\caption{\textbf{Shower test.} (a) M3 candidate-track count by spark class --- no-track
sparks are $\text{ncand}=0/1$, not $\ge2$; (b) the $\ge2$-track shower fraction is not
meaningfully enriched in sparks; (c) muon-tracked and no-track sparks have identical
discharge size (and span/duration/edge --- see text).}
\label{fig:shower}
\end{figure}

\section{Q2c --- Direct FEU1 (M3) cross-talk test}
To test the cross-talk directly, we decoded the raw M3 data (FEU1) for this run --- the
\texttt{.fdf} pulled from lxplus and processed with the cosmic-bench producer
(\texttt{DataReader \dots analyse}) --- giving, per event and per M3 layer, the cluster
count, the producer's own spark flag, and cluster amplitudes. Joined on \texttt{evn} to
our FEU7/8 sparks (matching verified at 100\,\% in the overlap window, 20\,827 events,
1\,568 sparks), the result (Fig.~\ref{fig:crosstalk}):
%
\begin{itemize}\setlength\itemsep{0.1em}
  \item \textbf{M3 is not knocked out.} It \emph{never} fires its own spark
        (\texttt{MGv2\_Spark}$=0$ in all 38\,934 decoded events) and it drops \emph{no}
        triggers on our sparks (100\,\% of our spark events are present in the FEU1 tree).
        So the M3 readout does not black out or discharge in sympathy.
  \item \textbf{But there is a real coincident excess.} When our detector sparks, M3
        records \textbf{+54\,\%} more clusters ($14.9$ vs $9.7$ per event, $\sim$5 extra),
        and those extra clusters are \textbf{lower-amplitude and smaller} (median amplitude
        521 vs 607, size 4 vs 5 strips) --- the signature of low-level \textbf{noise pickup}
        in the shared readout, not real extra particle tracks.
\end{itemize}
%
So the cross-talk is \textbf{confirmed but mild}: a spark injects a handful of spurious
low-amplitude clusters into M3, enough to spoil the straight-line track fit (extra hits
worsen $\chi^2\to$ track rejected, i.e.\ the $17.5\to34\,\%$ blank excess), but far from a
readout blackout. In the noisier ZS runs the same coupling instead shows up as
spark-correlated \emph{dropped} triggers ($25$--$45\,\%$; audit note); this clean non-ZS
run shows only the mild cluster contamination. Together with the muon-free spontaneous
component, this fully accounts for the no-track sparks --- no showers, no mystery.

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{fig_crosstalk.png}
\caption{\textbf{FEU1/M3 cross-talk.} (a) M3 never drops a trigger nor fires its own spark
on our sparks (both 0\,\%); (b) yet it records $+54\,\%$ more clusters when we spark;
(c) the excess clusters are low-amplitude --- noise pickup, not tracks.}
\label{fig:crosstalk}
\end{figure}
...
\section{Spark hitmaps --- and a second, detector-side shower check}
Fig.~\ref{fig:hitmap} maps where sparks live in the detector plane. The discharge
\emph{centre} (mean of a spark's fired strips) is a central blob (a), but the
\emph{footprint} hitmap (b) --- the fraction of sparks with coincident X\,\&\,Y activity
in each cell --- is a striking fixed \textbf{grid}: bright bands along all four edges plus
a few fixed hot strips, the same cells lighting up spark after spark, over a dark
($\sim$5\,\%) interior. This fixed footprint is itself a shower veto: a shower would land
where the muon flux does (uniformly, as in Fig.~\ref{fig:muon}b), \emph{not} repeatedly on
the same edges and hot channels. The edges/hot-strips are a detector property.

The fired-strip micro-structure (Fig.~\ref{fig:clusters}) says the same thing from the
mx17 side, addressing the concern that a \emph{tight} shower (which M3 could not resolve)
might mimic a spark. A spark is a few broad blobs (median 4 contiguous clusters per plane,
patchy fill $\sim$0.19) spread over $\sim$half the plane --- but the crucial number is the
\textbf{widest contiguous cluster: median 19~mm} (Fig.~\ref{fig:clusters}c), with no
population at the $\sim$5--8~mm width of a single-particle track. Real particle tracks, however
tightly bunched, would show up as $\sim$mm-wide clusters; the spark blobs are $2$--$4\times$
wider and sit at fixed positions. So even a collimated shower is excluded: the detector
sees discharge blobs, not tracks.

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{fig_hitmap.png}
\caption{\textbf{Spark hitmaps} (detector strip frame). (a) discharge centre (mean of
fired strips per spark); (b) footprint --- \% of sparks with X\,\&\,Y activity per cell,
a fixed edge$+$hot-strip grid; (c) per-plane occupancy, edges over-weighted.}
\label{fig:hitmap}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{fig_clusters.png}
\caption{\textbf{Detector-side shower check.} (a) contiguous clusters per spark; (b) fill
fraction; (c) widest contiguous cluster --- median 19~mm, far wider than a $\sim$mm
particle track, so the fired strips are discharge blobs, not tight tracks.}
\label{fig:clusters}
\end{figure}
...
\begin{itemize}\setlength\itemsep{0.15em}
  ...
  \item \textbf{Not showers:} the ``no-track'' sparks are not multi-particle showers ---
        only $1.3\,\%$ carry $\ge2$ M3 tracks and their discharge morphology is identical
        to muon-tracked sparks (Q2b).
  \item \textbf{Cross-talk confirmed (mild), from the M3 raw data (Q2c):} a spark injects
        $\sim$5 extra low-amplitude clusters into the shared FEU1 readout ($+54\,\%$), enough
        to spoil the M3 track fit ($17.5\to34\,\%$ blank), but M3 never sparks or drops a
        trigger. So the no-track sparks are cross-talk-degraded tracks $+$ muon-free
        spontaneous sparks --- fully explained, no showers.
  \item \textbf{Follow-ups:} (1) inspect the edge / guard-ring field and the Y/FEU8 hot
        strips (mask or lower gain); (2) mitigate the cross-talk (grounding/shielding of the
        shared DAQ) --- it is worse in ZS runs (spark-correlated dropped triggers
        $25$--$45\,\%$); (3) an offline spark tag (multiplicity $+$ the $\sim$1~\textmu s
        duration signature) would clean the reco\_far tail (companion note
