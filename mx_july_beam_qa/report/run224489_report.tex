...
\emph{first plastic-MIP calibration}: MIP amplitudes at nominal HV span
3.7\,mV (DR) to 14.3\,mV (CL), i.e.\ 0.73--2.83\,mV/MeV against the expected
5.05\,MeV deposit in 2.5\,cm of PVT; the DR and BL MIPs sit at or below the
recommended 4.9\,mV trigger threshold. (3)~The FIFO recovers only
...
9.2\,k (CL) to 49.5\,k (BR). The subtracted plastic spectra show a clear hump
that marches with HV (Fig.~\ref{fig:mipspectra}) --- a real peak, well above
threshold for $V\geq1400$\,V, sinking into the threshold turn-on at the
lowest steps. As a systematic check, adding the LIQ tag leaves the \emph{wall}
spectrum in triples unchanged (Fig.~\ref{fig:wallintriples}): the third tag
purifies without biasing.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth]{19_triples/pss_mip_spectra.png}
\caption{Triple-tagged plastic spectra per HV step (double
sideband-subtracted, smoothed). The MIP hump moves coherently with HV; the
red curve is the nominal-HV window.}
...
Per-step MIP modes are noisy ($\sim$2\,k net triples per PMT per step), so the
modes measured at $V\geq1400$\,V --- where the peak is well separated --- are
transported to the nominal voltage using each PMT's high-statistics
coincident-median exponent $n$ (Fig.~\ref{fig:mipvsv},
Table~\ref{tab:mip}). The internal spread of the transported values is
10--25\% (CL 78\%, inflated by one outlier step).

\begin{figure}[H]\centering
\includegraphics[width=\textwidth]{19_triples/pss_mip_vs_v.png}
\caption{Plastic MIP mode vs.\ HV with the coincident-median power law
anchored to the trusted ($V\geq1400$\,V) modes; the star is the nominal-HV
MIP, the gray cross the (threshold-biased) directly measured nominal mode.}
\label{fig:mipvsv}
\end{figure}
...
\begin{tabular}{lrrrrrr}
\toprule
PMT & $V_{\rm nom}$ [V] & $n$ & MIP [ADC] & MIP [mV] & spread & mV/MeV\\
\midrule
PSSA1 (AL) & 1325 & 6.94 & 376 & 11.5 & 20\% & 2.28\\
PSSA2 (AR) & 1275 & 5.08 & 288 &  8.9 & 10\% & 1.76\\
PSSB1 (BL) & 1325 & 5.19 & 147 &  4.5 & 19\% & 0.89\\
PSSB2 (BR) & 1300 & 7.10 & 275 &  8.4 & 24\% & 1.66\\
PSSC1 (CL) & 1300 & 3.80 & 465 & 14.3 & 78\% & 2.83\\
PSSC2 (CR) & 1300 & 6.64 & 259 &  8.0 & 22\% & 1.59\\
PSSD1 (DL) & 1300 & 5.77 & 169 &  5.2 & 21\% & 1.03\\
PSSD2 (DR) & 1300 & 6.39 & 119 &  3.7 & 15\% & 0.73\\
\bottomrule
\end{tabular}
\caption{Plastic MIP at nominal HV and absolute calibration. The energy scale
assumes the normal-incidence minimum-ionizing deposit in 2.5\,cm of PVT
...
\textbf{at the current HV the DR (3.7\,mV) and BL (4.5\,mV) MIPs are at or
...
\item The plastic MIP is measured for the first time:
      0.73--2.83\,mV/MeV at nominal HV. \textbf{Apply the new equalization}
