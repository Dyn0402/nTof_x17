...
\begin{frame}{Do the three plastic equalizations agree?}
\begin{columns}
\column{.6\textwidth}
\includegraphics[width=\textwidth]{21_y88/equalization_compare.png}
\column{.4\textwidth}
Three equalizations, all normalized to fleet-mean 1300 V:
\begin{itemize}
\item 224466 median (BNC-T) $=$ \textbf{current fixed voltages};
\item 224489 median (FIFO); \; Y-88 699 keVee edge (BNC-T).
\end{itemize}
\vspace{0.5ex}
\begin{itemize}
\item \textbf{Same broad pattern} (CL lowest, DR highest); agree to
      \textbf{$\sim\pm25$ V RMS}.
\item Y-88 edge vs current: RMS \textbf{23 V}; FIFO median vs current: 31 V.
\item Mostly they point the same way (AL/AR down, BL/DR up)\ldots
\item \ldots\ but \textbf{DL and CR disagree}: the MIP-tagged median and the
      0.7 MeV edge want \emph{opposite} re-tunes ($\Rightarrow$ the equalization
      \emph{observable} matters $\sim$25--80 V for those tubes; mild
      nonlinearity).
\end{itemize}
\end{columns}
\end{frame}
...
\begin{frame}{\textbf{Recommended plastic HV: Y-88-edge equalization (FIFO)}}
Equalize each PMT's \textbf{699 keVee edge} (slid along its own power law) to a
common target; target chosen to keep the \textbf{fleet-mean HV at 1301 V}
(overall gain unchanged). Y-88 is BNC-T, so folded in the per-PMT FIFO factor.
\vspace{1ex}
\begin{center}\small
\begin{tabular}{lcccccccc}
\toprule
 & AL & AR & BL & BR & CL & CR & DL & DR \\
CAEN 07.\_ & .000 & .001 & .002 & .003 & .004 & .005 & .006 & .007 \\
\midrule
current V     & 1303 & 1242 & 1376 & 1279 & 1180 & 1307 & 1303 & 1417 \\
\textbf{set V}& \textbf{1237} & \textbf{1177} & \textbf{1440} & \textbf{1248} & \textbf{1214} & \textbf{1312} & \textbf{1331} & \textbf{1448} \\
...
\vspace{1ex}
\begin{itemize}
\item Arm A comes \emph{down} ($\sim$65 V): it has the highest FIFO factor
      (1.65/1.56), so it over-gains in FIFO. BL/DL/DR/CL come up.
\item Range 1177--1448 V (HV scans ran to 1600 V fine).
\item Machine-readable: \texttt{calib/plastic\_hv\_equalized\_y88.json}.
      Target is a scale knob --- raise it to shift all gains up together.
\end{itemize}
\end{frame}

\begin{frame}{The one caveat, explained: response \textbf{nonlinearity}}
\includegraphics[width=\textwidth]{21_y88/nonlinearity_concept.png}
\vspace{-0.5ex}
\begin{itemize}\small
\item We equalize so every PMT's \textbf{699 keVee edge} reads the same mV.
\item \textbf{If the response were linear} (signal $\propto$ energy, through the
      origin): matching at 699 keVee makes the whole response curves identical,
      so the MIP is \emph{automatically} equal too --- equalize once, done (left).
\item \textbf{If it is nonlinear} (the curve bends, and by a different amount per
      PMT): the curves cross at 699 keVee but \emph{diverge} at higher energy, so
      the MIP is \emph{not} equal even though the edge is (right).
\item Real scintillator+PMT chains bend a little (light collection, saturation).
\end{itemize}
\end{frame}

\begin{frame}{The nonlinearity, in our data}
\includegraphics[width=\textwidth]{21_y88/nonlinearity_data.png}
\vspace{-0.5ex}
\begin{itemize}\small
\item \textbf{(a) Same-run check:} the two Y-88 edges (699 \& 1612 keVee). If
      linear, signal(1612)/signal(699) $=$ \textbf{2.307} for \emph{every} PMT;
      measured 1.8--2.6 (the 1612 edge is the weaker fit, so read the per-PMT
      values loosely --- but the spread is real).
\item \textbf{(b) Robust, higher lever:} MIP-scale median $/$ 699 keVee edge.
      Linear $\Rightarrow$ one constant for all 8 PMTs; measured
      \textbf{1.70--2.38 ($1.4\times$)}. Arm A / DL sit $\sim$15--20\% low ---
      those tubes \textbf{compress} more at high energy.
\item Same story as slide~6 (median vs edge equalizations disagree on DL/CR) and
      the persistent arm-A-low MIP.
\end{itemize}
\end{frame}

\begin{frame}{How to deal with the nonlinearity --- suggestions}
The effect is \textbf{small ($\sim\pm12\%$ at the MIP)}. Options, in order of
effort:
\begin{enumerate}
\item \textbf{Accept it (recommend now).} Equalize on the Y-88 edge (absolute,
      known energy). MIP/trigger then uniform to $\sim\pm12\%$. Set a common
      0.5-MIP threshold $\sim$76 mV; per tube it is 63--88 mV.
\item \textbf{Per-PMT thresholds} (small bookkeeping): keep the Y-88
      equalization but set each tube's threshold to its own 0.5-MIP
      (\textbf{63--88 mV}) $\Rightarrow$ the \emph{trigger point} is uniform even
      though the gain isn't, exactly.
\item \textbf{Equalize at the trigger energy} instead of the edge: use the
      MIP/median scale ($\sim$1.7 MeV, closest to the trigger). Uniform trigger,
      but you lose the absolute anchor and re-inherit the median's caveats.
\item \textbf{Characterize \& correct the full curve}: a multi-point calibration
      (more source lines / a dedicated MIP+edge study) per PMT. Only worth it if
      a physics analysis needs $<5\%$ linearity everywhere.
\end{enumerate}
\vfill
{\small \textbf{My recommendation:} (1) now --- the $\pm12\%$ is small and keeps
the absolute scale; add (2) if the trigger uniformity matters more than the
absolute energy scale. (4) only on physics demand.}
\end{frame}
