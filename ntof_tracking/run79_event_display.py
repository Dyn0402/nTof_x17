...
# -*- coding: utf-8 -*-
"""
run79_event_display.py -- event displays for the PRELIMINARY run_79
...
# ------------------------------------------------------------- the waveforms
def refit_event(event_id, arm='A', run='run_79', sub_run='stat090_0000'):
    ...
    for tag in wb.subrun_tags(cfg):
        ...
        for plane in ('x', 'y'):
            ...
            for i, P in enumerate(cand):
                ...
                key = wreco._cand_key(plaus, dchi2, f)
...
def fig_waveforms(ev, event_id, arm, out_path, v_drift=36.0):
    ...
    for ri, plane in enumerate(('x', 'y')):
        ...
        ax.text(.98, .95, f'p0 = {f.p0:.1f} mm\ntan = {f.tan_theta:+.3f}'
                ...
                color=MUTED)
    fig.suptitle(f'run_79 / mx17_{arm}  event {event_id}: the waveforms and the '
...
def fig_projections(row, g, arm, event_id, out_path):
    ...
    fig.suptitle(f'run_79 / mx17_{arm}  event {event_id}: waveform-first track '
...
def _compose_3d(imgs, labs, g, arm, event_id, out_path):
    ...
    fig.suptitle(f'run_79 / mx17_{arm}  event {event_id}: the same track in the '
