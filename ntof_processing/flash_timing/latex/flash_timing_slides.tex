...
{\footnotesize \hl{Per-wall means, for orientation only.} The calibration is
\textbf{per channel} --- 32 constants, 07-16 epoch, runs $\ge224400$.}
\vspace{0.4em}

{\footnotesize
\begin{tabular}{lrr}
\toprule
scheme & rms resid. & max \\
\midrule
one constant, all 32 & 3.43 & 7.18 \\
one per wall & 3.42 & 7.07 \\
\textbf{per channel} & \textbf{0} & \textbf{0} \\
\bottomrule
\end{tabular}}
\vspace{0.3em}

{\footnotesize Wall means agree to $0.7\ns$; channels within a wall differ by up
to $13.3\ns$ $\Rightarrow$ grouping by detector captures \hl{nothing}.}
\end{column}
...
\begin{itemize}
  \item pickup $0$; walls $-5$; plastics $-3\ldots-6$; \hl{liquids $-4.1\ldots-4.9$};
        silicon \hl{$-16\ns$}. The scintillators cluster where ``more light
        $\rightarrow$ earlier crossing'' predicts; a real change of the flash
        \emph{arrival} would have to move the pickup too.
...
\begin{itemize}
  ...
  \item common-mode wander \hl{$0.68\ns$ rms}, worst single run $-2.2\ns$
        (224531); at \emph{fixed} intensity it tightens to
        \hl{$0.41$--$0.76\ns$} (LIQA/B/C)
...
\begin{enumerate}\setlength\itemsep{0.55em}
  \item \textbf{Walls}: per-channel \texttt{C\_2026\_07\_16} (\hl{all 32}) for
        runs $\ge224400$, \texttt{C\_2026\_07\_11} before --- never one constant
        per wall. Add $-5\ns$ if the pulse type differs from the calibration one.
