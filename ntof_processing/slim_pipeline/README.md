...
[3] pass 2  every scintillator hit within +-150 ns of the FULLY CORRECTED
...
Measured containment of the background-subtracted excess (sig − ctl):
...
±150 holds the wall coincidence completely, 92 % of the liquid excess, and
leaves ~6× the accept window of flat sideband. The choice is one constant,
`config.SLIM_NS`; re-slimming a sub-run costs ~6 minutes.

**Known, unexplained: the plastics have a long LATE tail.** The sig/ctl ratio
per 25 ns bin is flat at 1.03 for dt < −150 ns but is still **4.2** at
dt = +238 ns and falling slowly. It is one-sided, so it is not a control
rate-gradient artifact. Most likely the PSS shape fitting (101 ns templates,
v11/v12) splitting long plastic pulses, or genuine afterpulsing. It is
truncated at any window we would reasonably choose. Worth characterising once
from the source; it does not affect the trigger match or the liquid physics.
