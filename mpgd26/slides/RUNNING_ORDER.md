...
| 15 | **Every volt of gain costs milliseconds — and the milliseconds are the beam** *(a six-frame build since 2026-08-24)* | 80 s | **Rebuilt 2026-08-24** (Dylan). The charge ladder and the dead-time-vs-charge plot are both in backup now; the slide is the X17-rate-vs-flight-time drawing with **each HV setpoint’s measured recovery drawn as the wall it really is**, built **560 → 550 → 540 → 530 → 520 V and then back to 540**. Three panels: **the cosmic bench’s own detection efficiency** vs HV (the 27 June det3 scan, carried across the 95/5 → 90/10 gas boundary), three large numbers on a post-it (voltage, **per cent of the X17 rate left**, efficiency), and charge vs the recovery it buys on the main panel’s own time axis. **Two sentences you now have to say, because the figure stopped saying them (2026-08-28).** The top panels lost their headlines when they were stretched into the space: say **“the cosmic bench’s own efficiency, mapped onto the n_TOF voltage axis”** for the left panel — its y axis says only `Efficiency [%]`, and *bench* and *mapped* are the whole claim — and **“detector A, run_57, one sub-run every 2 V”** for the charge strip, which now names nothing about its own provenance. **Linger on frame 6** — it is the same drawing as frame 3 with every other voltage’s edge left on the axis, which is where you say we chose this. The two costs multiplied (optimum 550 V, 540 V at 95 % of it) was frame 7 until 2026-08-24 and is **backup only** now (Dylan: “kick 25.7 to backup”) — bring it up if the setpoint is questioned. Numbers: `ntof_july_analysis/hv_tradeoff/report.html`. **Three questions this panel invites.** *Which scan?* — 27 June saturday det3, 425–525 V, the only one reaching below the plateau; the 22 June one starts already flat and both are on backup D2c. *Why is 540 V only 69 %?* — the **23 July noise step**. The bench curve is placed on the n_TOF axis by the full threshold ledger, and the production front end needs 22 V more gas gain than run_55’s did for the same signal-to-noise; that 22 V is what turns 540 V from 81 % into **69 %**. Until 2026-08-28 both placements were drawn and the gap between them *was* the answer; the July curve was removed for legibility and the number is now yours to explain. (`T.bench_eff_on_ntof_axis('run_55', …)` still returns it.) *Is 520 V measured?* — no, it maps to bench 417 V and its number is marked `~` and extrapolated. **The run_55 MIP-track ladder is NOT an efficiency** (doubles trigger, ~50 % geometric ceiling, a 3-strip cluster in both views) and is backup D2b. |
