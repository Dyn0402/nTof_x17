...
RUN=run_145
SUB=stat090_0002
OUT=/media/dylan/data/x17/sept26_prelim/fullpass/$RUN/$SUB
JOBS=${JOBS:-14}
LOG=$OUT/pass.log
...
for ARM in A B C D; do
  ...
  .venv/bin/python -W ignore -m ntof_tracking.wft_beam reco \
      --det "$ARM" --run "$RUN" --subrun "$SUB" --jobs "$JOBS" \
      --bundle "/media/dylan/data/x17/sept26_prelim/stage2/reco_run145_stat090_0000/mx17_$ARM/calib_bundle_prelim" \
